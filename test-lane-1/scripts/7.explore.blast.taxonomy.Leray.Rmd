...
Explorative plots looking at taxonomic assignments from **Leray** sequences.
...
indir_asv    <- 'test-lane-1/data/dada2/Leray'
indir_meta   <- 'test-lane-1/data/cutadapt/Leray'
...
infile1 <- 'lane1_leray_blast_uniqueIDs_taxonomy.csv'

# hashes with multiple IDs, that required the LCA function
infile2 <- 'lane1_leray_blast_multipleIDs_taxonomy.csv'
```
<br>
<br>
...
These are true duplicates - I'm not sure why they weren't filtered out.
```{r}
checkdat <- dat %>% 
  group_by(Sample_name,Hash,dataset) %>%
  summarise(count=n(), .groups="drop") %>%
...
```
<br>
The proportion of reads without taxonomic assigments is crazy low. Except for WACO_2021_003a, which looks like it will have no data. 
<br>
...
It looks like the green crab DNA once again got...everywhere. The rest is mostly bacteria and fungus (whereas BF3 had only bacteria). The weirdest thing is that there isn't any kangaroo DNA in the positive control??

Non-bacterial species in the negative control:

1. *Planosiphon gracilis*, a brown algae

2. *Heterosigma akashiwo*, a unicellular red algae

3. *Stegodyphus*, a spider

4. *Sydowia polyspora*, a plant pathogen that infects Douglas firs

5. *Bangiophyceae* (class; phylum=Rhodophyta), a red algae

6. *Dothideomycetes*, a fungus

7. *Candida viswanathii*, a yeast
...
Weird, it almost looks like WACO_2021_002a should be the positive control???
...
ggplot(data=filter(plotdat, phylum %in% c("Arthropoda","Mollusca")), aes(x=SampleID, y=pReads, fill=taxon)) +
  geom_col() + facet_grid(col=vars(Group), rows=vars(dataset), scales="free") +
  ggtitle("Arthropods & Molluscs") +
  theme_bw() + theme(axis.text.x=element_text(angle=90,hjust=1,vjust=0.5))
```
<br>

Similar to BF3, we have lots of Dungeness crab and *Hemigrapsus* reads (plus crab that couldn't be ID'd to genus or species, *Malacostraca*, *Cancridae*, *Carcinus*, *Liocarcinus*). And some things must be contamination, or non-specific primer matches from having such short sequences (Lepidoptera, Mermessus spp, Empoasca). But there are also some other cool things: 

- amphipods: *Ampithoe lacertosa*  (also picked up in BF3)
- shrimp: *Crangon* (not in BF3)
- barnacles (*Balanus*)
- bivalves (*Ruditapes philippinarum*) -yay!!!


Weird thing about the bivalve results between Leray and BF3  -- same species, but different crabs?? 


<br>

```{r echo=FALSE}
ggplot(data=filter(plotdat, phylum %in% c("Bacillariophyta","Rotifera","Oomycota","Ascomycota")), aes(x=SampleID, y=pReads, fill=taxon)) +
  geom_col() + facet_grid(cols=vars(Group), rows=vars(dataset), scales="free") +
  ggtitle("Algae, Rotifers, and Fungi") +
  theme_bw() + theme(axis.text.x=element_text(angle=90,hjust=1,vjust=0.5))
```
<br>

```{r echo=FALSE}
ggplot(data=filter(plotdat, phylum %in% c("Cnidaria","Annelida","Porifera")), aes(x=SampleID, y=pReads, fill=taxon)) +
  geom_col() + facet_grid(cols=vars(Group), rows=vars(dataset), scales="free") +
  ggtitle("Sponges, Hydroids, and Worms") +
  theme_bw() + theme(axis.text.x=element_text(angle=90,hjust=1,vjust=0.5))
```
...
write.csv(egc_prey_summary, here(outdir,"Leray_BLAST_prey_taxa_EGC.csv"), row.names=FALSE)
```
<br>
<br>
...
ggplot(plotdat, aes(x=SampleID, y=pReads, fill=taxon)) +
  geom_col() +
  facet_wrap(~dataset) +
  ggtitle("Taxa") +
  theme_bw() + theme(axis.text.x=element_text(angle=90,hjust=1,vjust=0.5))
...
Well, crap, it looks like there's just a bunch of contamination from the green crab. Plus some brown algae (*Hecatonema maculans*)
...
write.csv(dungy_prey_summary, here(outdir,"Leray_BLAST_prey_taxa_Dungeness.csv"), row.names=FALSE)
```
<br>
<br>
