...
```
<br>
Forward reads tend to have fewer successful taxonomic IDs, except for several technical replicates for one WACO 2021 crab / two Dungies / one WASS 2020 crab. Merged reads tend to have the most successful taxonomic IDs.
Across all crab, there are a very high proportion of hashes that aren't successfully assigned a taxonomy, but usuall <= 25% of reads. I have a feeling that's because a large number of reads come from very few hashes -- especially hashes that come from predator DNA.
<br>
...
Algae species in the negative controls:

1. *Planosiphon gracilis*, a brown algae

2. *Pseudoralfsia verrucosa*, a brown algae



Algae species only in the kangaroo:

1. 68 reads of *Bangiophyceae* (class; phylum=Rhodophyta), a red algae, in kangaroo


Get a list of taxa from the negative controls, to remove from the samples:
```{r}
taxa_to_remove <- control_dat %>%
  filter(SampleID %in% c("PCRNegative","SpeedVacNegative")) %>%
...
ggplot(data=filter(plotdat, phylum %in% c("Arthropoda","Mollusca","Cnidaria")), aes(x=SampleID, y=pReads, fill=taxon)) +
...
So we have lots of Dungeness crab and *Hemigrapsus* reads (plus crab that couldn't be ID'd to genus, *Malacostraca*). But there are also some other cool things: 

- amphipods: *Ampithoe lacertosa*
- copepods: *Amphiascopsis cinctus*
- ostracods: *Cyprideis torosa*
- worms: *Algaophenia tubulifera*
- barnacles (*Balanus*)
- bivalves: *Mya spp.* are soft-shell clams; *Ruditapes* are little-neck clam (yay!!!!)

Also, really crazy-- *portunion conformis* is a parasitic isopod, often found in shore crab hosts
<br>

```{r echo=FALSE}
ggplot(data=filter(plotdat, phylum %in% c("Bacillariophyta","Chlorophyta")), aes(x=SampleID, y=pReads, fill=taxon)) +
  geom_col() + facet_grid(cols=vars(Group), rows=vars(dataset), scales="free") +
...
write.csv(egc_prey_summary, here(outdir,"BF3_BLAST_prey_taxa_EGC.csv"), row.names=FALSE)
```
<br>
<br>
...
write.csv(dungy_prey_summary, here(outdir,"BF3_BLAST_prey_taxa_Dungeness.csv"), row.names=FALSE)
```
<br>
<br>
