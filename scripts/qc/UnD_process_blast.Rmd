...
User directories
```{r}
indir    <- 'data/raw/run2/undetermined/blast'
outdir   <- 'data/raw/run2/undetermined/blast'
dada_dir <- 'data/raw/run2/undetermined/dada2'
```
<br>

Files containing Blast output.
```{r}
blast_filenames     <- c('undetermined_hash_key_blast_2022-09-28.fasta')
filename_identifier <- c('undetermined')
```
<br>

Output files
```{r}
consensus_out <- 'leray_undetermined_blast_multipleIDs_taxonomy.csv'
unique_out    <- 'leray_undetermined_blast_uniqueIDs_taxonomy.csv'
all_out       <- 'leray_undetermined
._blast_taxonomy.csv'
```
<br>
...
```{r}
hash.meta <- read_csv(here(dada_dir, 'undetermined_ASV_table.csv'))
```
<br>
...
```
This is because I deleted the Hash key information for all samples that weren't "undetermined" Sample 0, but processed them all in dada2 at the same time.
...
So 51.8% of hashes have taxonomy assigned (118 out of 228) but just 0.745% of reads have taxonomy assigned (7517 out of 100937)
<br>
...
```{r}
sample_taxonomy %<>% filter(Sample_name==0)
write.csv(sample_taxonomy, here(outdir, paste0(locus,'_',run.num,"_sample_blast_lca_taxonomy.csv")), row.names=FALSE)
```
<br>
...
I'm only interested in whether there is prey information in these un-indexed reads. So I need to get rid of: 

1. Kangaroo (and human)

2. Green crab (predator)

3. Bacteria, Amoeba, etc.

4. Small ocean critters (diatoms, rotifers, etc.)

```{r}
sample_taxonomy %<>% filter(!is.na(rank))
```
<br>
...
```{r}
blast.dat.filter <- sample_taxonomy %>% 
  mutate(control_rm=ifelse((order == "Diprotodontia" | order =="Primates" | (class=="Mammalia" & is.na(order))),1,0),
         predator_rm=ifelse(control_rm==0 & (taxon =="Carcinus maenas" | taxon =="Carcinus aestuarii" | taxon =="Carcinus"), 1, 0))
```
<br>
<br>
...
to_remove_class <- c("Dinophyceae","Choanoflagellata")
to_remove_taxa <- c("Hematodinium sp.","Naegleria","Euamoebida","Amoebozoa sp.","Monosiga brevicollis")
...
to_remove_taxa  <- c("Cymbomonas tetramitiformis","Micromonas pusilla","Planosiphon gracilis","Paralia sulcata")
...
### inexact tax ids
```{r}
blast.dat.filter %<>% mutate(inexact=ifelse(!(rank %in% c("family","genus","species")),1,0))
```
<br>

## review & save

it does look like there is some prey info in here, so it's worth trying to trouble shoot the lack of recognized indexes on these reads.

```{r}
dat.out <- filter(blast.dat.filter, control_rm==0 & predator_rm==0 & baf_rm==0 & too_small_rm==0 & inexact==0)

with(dat.out, table(taxon,nReads))
```
<br>


```{r}
write.csv(blast.dat.filter, here(outdir,'leray_undetermined_sample_blast_processed.csv'))
```
