---
title: "DADA2 for Undetermined Reads"
...
Used to explore the reads that were not indexed *but* did have a Leray primer on them. I copy and pasted the cutadapt output from the indexed samples into the file folder, so that I could run all the samples together with the "undetermined" samples. Then I'll just blast the Undetermined ASVs.
...
cutadapt_dir <- "data/raw/run2/undetermined"
# output directory
outdir <- "data/raw/run2/undetermined/dada2"
```
...
Output file names
```{r}
conv_file <- here(outdir,"undetermined_hash_key.csv")
conv_file.fasta <- here(outdir,"undetermined_hash_key.fasta")
ASV_file <-  here(outdir,"undetermined_ASV_table.csv")
```
<br>
...
Double check the quality scores *if not already done with QC script 0*
...
output.dada2.und <- output.dada2 %>% filter(Sample_name==0)
write.csv(output.dada2.und, file=here(outdir,"UnD_dada2_output.csv"))
```
...
```
```{r}
output.dada2 <- output.dada2 %>%
  mutate( errF1 = map(filtF1, ~ learnErrors(.x, multithread=FALSE,verbose = 0)),     # Calculate errors
          errR1 = map(filtR1, ~ learnErrors(.x, multithread=FALSE,verbose = 0)),
          derepF1 = map(filtF1, derepFastq),                   # dereplicate seqs
          derepR1 = map(filtR1, derepFastq),
          dadaF1  = map2(derepF1,errF1, ~ dada(.x, err = .y, multithread = FALSE)),  # dada2
...
output.dada2.und <- output.dada2 %>% filter(Sample_name==0)
seqtab <- makeSequenceTable(output.dada2.und$mergers)
         
write.csv(table(nchar(getSequences(seqtab))) %>% as.data.frame() %>%
  mutate(Length=as.character(Var1),
         Length=as.numeric(Length)), file=here(outdir,'dada2_filtered_seqLengths_undetermined.csv'))
```
Sample 1 - 1019744 reads in 95941 unique sequences.
...
Sample 1 - 1019744 reads in 103140 unique sequences.
...
Error: object 'keep.mid.files' not found
...
filter for only undetermined reads
```{r}
output.dada2.und <- output.dada2 %>% filter(Sample_name==0)
seqtab <- makeSequenceTable(output.dada2.und$mergers)
