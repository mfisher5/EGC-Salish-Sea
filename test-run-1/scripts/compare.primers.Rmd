...
nontarget_taxa <- c("Actinobacteria","Bacteroidetes","Planctomycetes","Proteobacteria","Discosea", "Tubulinea")
...
What if we consider DNA from rotifers, algae, etc. as all "non-target"? This is relevant only for the green crab samples. *Still BF3*
```{r}
nontarget_taxa2 <- c(nontarget_taxa, "Bacillariophyta","Chlorophyta","Rhodophyta","Ascomycota","Rotifera","Oomycota")
nontarget_taxa2
```
```{r echo=FALSE, message=FALSE, warning=FALSE, fig.height=3,fig.width=8}
nontarget_dat2 <- all_dat2 %>%
  group_by(Locus,site) %>%
  summarise(total_reads=sum(nReads), .groups="drop") %>%
  left_join(filter(all_dat2, phylum %in% nontarget_taxa2) %>%
              group_by(Locus,site) %>%
              summarise(nontarget_reads=sum(nReads), .groups="drop")) %>%
  mutate(preads=nontarget_reads/total_reads)

ggplot(nontarget_dat2,aes(x=Locus,y=preads)) +
  geom_col() +
  facet_grid(cols=vars(site)) +
  ylab("Proportion Reads\n Non-Target DNA") +
  theme_bw()
...
But mostly because the bacteria / amoeba reads from BF3 outnumber the algae, rotifer reads from Leray/LerayXT (note the Y axis):
```{r echo=FALSE, message=FALSE, warning=FALSE, fig.height=3,fig.width=8}
nontarget_taxa3 <- c("Bacillariophyta","Chlorophyta","Rhodophyta","Ascomycota","Rotifera","Oomycota")
nontarget_dat3 <- all_dat2 %>%
  group_by(Locus,site) %>%
  summarise(total_reads=sum(nReads), .groups="drop") %>%
  left_join(filter(all_dat2, phylum %in% nontarget_taxa3) %>%
              group_by(Locus,site) %>%
              summarise(nontarget_reads=sum(nReads), .groups="drop")) %>%
  mutate(preads=nontarget_reads/total_reads)

ggplot(nontarget_dat3,aes(x=Locus,y=preads)) +
  geom_col() +
  facet_grid(cols=vars(site)) +
  ylab("Proportion Reads\n Algae, Rotifer, Fungal DNA") +
  theme_bw()
```
<br>


Just take out the bacterial / amoeba dna for now.
...
Are the read depths from the 2018 samples noticeably different from the 2020 / 2021 samples? **Yes.**

```{r echo=FALSE, message=FALSE, warning=FALSE}

rename_samples <- read_csv(here::here("test-lane-1","data","cutadapt","misnamed_samples.csv"), col_types="cc")

rd_dat <- read_csv(here::here("data","raw","qc","EGC-Salish-Sea_Lane-1_MiSeq-Stats.csv"),col_types="ccii") %>%
 left_join(rename_samples,by=c("sample"="misnamed_sample")) %>%
  mutate(corrected_sample=ifelse(!is.na(corrected_sample),corrected_sample,sample)) %>%
  dplyr::select(-sample) %>%
  rename(sample=corrected_sample)

rd_dat %<>% separate(col=sample, into=c("primer","group","year","id1","id2"), sep="_", remove=FALSE) %>%
  ...
  filter(group!="PCRNegative" & group!="SpeedVacNegative" & group!="MARPT") %>%
    mutate(group_year=paste0(group,"_",year))

rd_dat$group_year <- factor(rd_dat$group_year, 
                    levels=c("WASS_2018","WASS_2020","WACO_2021","Kangaroo_NA"))

### plot
ggplot(rd_dat, aes(x=group_year, y=total_reads_per_lane/1000, fill=year)) +
  geom_boxplot(alpha=0.8) +
  geom_hline(aes(yintercept=70), lty=2, col="grey45") +
  geom_point(aes(fill=year), alpha=0.4) +
  facet_wrap(~primer, ncol=3) + 
  labs(y="Reads per Sample (x1K)", x="Group",
       title="MiSeq Read Depth") +
...
3. predator DNA (**on par or less than WASS 2020**)
...
Can we resolve a similar proportion of ASVs down to lower taxonomic levels in the WASS 2018 samples? **Yes - in proportion ASVs of primary prey taxa. But difference goes away when looking at reads instead of ASVs**
