...
params <- data.frame(folder="/Users/elizabethandruszkiewicz/GoogleDrive/UW/GitHub/NextGenNEPA_EA/Output/classification_output/run4_20211117/MiFish",
                 Hash_key="test-lane-1/data/Leray/hash_key.csv",
...
Load the database
```{r}
tree <- read_rds(here(params$classifier))
```
<br>
...
```{r classify_split}
clasif.split.hashes <- classify (x = split.hashes.insect, tree = tree, cores = 4)

names(clasif.split.hashes) <- c('representative', 'taxID', 'taxon', 'rank', 'score', 'kingdom', 'phylum', 'class', 'order', 'family', 'genus', 'species')

clasif.split.hashes %>% 
  unite (family, genus, species, sep = "|", col = "taxa")
clasif.split.hashes %>% dplyr::count (rank) %>% arrange(desc(n))
```
...
not.clasif.hashes <- clasif.split.hashes %>% 
  filter (taxID == "1")
