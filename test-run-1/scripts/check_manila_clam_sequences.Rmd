---
title: "Check Manila Clam (R. philippinarum) sequences"
...
Re-run **DADA2** [tutorial here](https://benjjneb.github.io/dada2/tutorial.html) in order to match the amplicon sequence variant (ASV) output with input sequences. 

This will (hopefully) let me trace the ASV matched to Manila clam in blast, to raw sequence data. 
...
It's possible to match dada's ASVs to original sequences using a combination of the "map" and "denoised" values saved into the `dada` object. 
...
length(bf3.derep$uniques)==length(dadaFs$BF3_WACO_2021_003c_S18$map)
```
<br>
